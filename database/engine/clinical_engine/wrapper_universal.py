...
# -----------------------------------------
# BASE_PATH = carpeta clinical_engine
BASE_PATH = Path(__file__).resolve().parent

# BACKEND_PATH = carpeta masesora_backend (sube 3 niveles)
BACKEND_PATH = BASE_PATH.parent.parent.parent

# DATA_PATH = carpeta data dentro de masesora_backend
...
def load_config(config_slug: Optional[str], tool_base: Optional[str]) -> Dict[str, Any]:
    ...
    config_dir = BASE_PATH / "configuraciones" / tool_base
...
def run_symptom(symptom_id: str, inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 3. Normalizar inputs
        inputs["inputA"] = inputs.get("inputA") or inputs.get("input_a") or inputs.get("input_a_value")
        inputs["inputB"] = inputs.get("inputB") or inputs.get("input_b") or inputs.get("input_b_value")
        ...
        try:
            ...
            output["motor_output"] = motor_output
        except Exception as e:
            output["motor_error"] = str(e)
        ...
        try:
            ...
            output["optimizer_output"] = optimizer_output
        except Exception as e:
            output["optimizer_error"] = str(e)

        # 6. KPI + semáforo
        try:
            kpi = compute_kpi(symptom, inputs)
            semaforo = compute_semaforo(symptom, kpi)
            output["kpi_actual"] = kpi
            output["semaforo"] = semaforo
        except Exception as e:
            output["kpi_error"] = str(e)

        # 7. Frameworks asociados
        try:
            frameworks = get_frameworks(symptom.get("domain", ""))
            output["frameworks_asociados"] = frameworks
        except Exception as e:
            output["frameworks_error"] = str(e)
