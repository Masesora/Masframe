...
## LVI. SESIÓN 22 AGO 2026 (cont.) — NEURO-S2 y NEURO-S3: las dos cadenas se rompen por encima de C3

Se retoma el encargo abierto de §LV.M con los 15 criterios de §LV.L. Aplicado el PASO 0 a los dos síntomas, **los dos salen 🔴 de cadena y por el mismo sitio: C1→C2**. Por eso esta sesión no repite la pasada de ramas de NEURO-S1: arreglar tarjetas sobre una cadena que empieza mal es el trabajo tirado que avisa la regla de oro de la skill. Lo mecánico ya aprobado sí se aplica (§LVI.A); todo lo que es criterio queda propuesto y a la espera (§LVI.H).

### LVI.A — Lo mecánico, aplicado de una pasada (52 cambios, cero criterio nuevo)

Criterios 7, 10, 11 y 12 de §LV.L, que no dependen de ninguna decisión pendiente:

- **`filas_iniciales: 20 → 1`** en NEURO-S2.r1 — el peor hallazgo de UX del catálogo (§XLVIII.B: página de 14.094 px y 20 acciones fantasma en C4).
- **`vista:"tarjeta"`** en las 5 secciones que seguían siendo tabla cruda (r2, r3, r5, r6×2).
- **Ninguna opción pre-marcada**: primera opción vacía en las 20 columnas `opciones` que no son la Decisión, entre los dos síntomas.
- **Colisión de `ACCION_REGEX` en NEURO-S2.r5** — *"¿Genera decisión?"* competía con *"Decisión final"*. Renombrada a *"¿Sale algo en firme de ella?"*. Comprobado después: exactamente 1 columna accionable por sección en las 11 secciones nativas.
- **`no_sumar` en 22 columnas**: el TOTAL sumaba horas con euros y con recuentos — NEURO-S3.r3 sumaba **8 columnas distintas**. Ahora cada sección suma una sola columna, siempre la calculada y con `unidad`.

Round-trip del JSON idéntico · `validar_sintomas.py` 2 errores y 21 avisos, los mismos que antes (los 2 son los de UCI-S3, previos) · `tsc --noEmit` limpio. Commit: `masframe@3d5f209`. Frontend sin tocar: nada de esto necesita motor.

### LVI.B — NEURO-S2: la causa que marca el cliente le abre la herramienta de otra causa

`capa_2_options[i]` ↔ `r{i+1}` alinean bien. Lo que está desplazado es **C1 → C2**: el motor construye el ítem de C2 con `descripcion = c2Options[idx]` donde `idx` es el índice de la causa marcada en C1 (`TreatmentPage.tsx:3503`). Y en NEURO-S2 esos dos arrays cuentan historias distintas en el mismo orden:

| i | C1 dice | C2 le contesta | y C3 le abre |
|---|---|---|---|
| 0 | *tengo demasiados proyectos a medias* | Bloquear tiempo fijo para la estrategia | r1 Planificador de agenda semanal |
| 1 | *cada semana algo urgente descoloca lo planificado* | Cerrar los proyectos a medias antes de abrir uno nuevo | r2 Inventario de proyectos abiertos |
| 3 | *no tengo tiempo protegido para pensar* | Definir las 3 prioridades de la semana | r4 Tres prioridades semanales |

Es el mecanismo crítico de **RES-S1** (§XLVIII.B), aquí en el índice 0 — la causa que más se marca. El linter no lo ve: mide solapamiento de vocabulario agregado, no alineación por índice.

### LVI.C — NEURO-S3: seis dilemas de "A o B" contestados con Sí/No

`capa_2_decision` es *Árbol de Decisiones*, y el motor pinta cada `capa_2_options[i]` con dos botones **Sí / No** (`TreatmentPage.tsx:3537-3596`), donde cada *Sí* se convierte en la decisión comprometida y titula el frente de C3. Las 6 opciones de NEURO-S3 son disyuntivas: *"¿Mantener servicios de bajo margen para completar la oferta **o** eliminarlos aunque pierda algún cliente?"*. Contestar *Sí* a un "A o B" no significa nada, y ese texto es el que baja a C3 y a C4.

Es el patrón crítico de **OPE-S3** (§XLVIII.B) en otra familia. Y es aislable: NEURO-S3 es **el único** de los 6 síntomas de familia árbol que lo hace — CARDIO-S3, CLI-S3, PSI-S2, RES-S3 y OPE-S2 usan todos el patrón de la casa, *"¿[acción concreta] aunque [coste]?"*, que sí se contesta Sí/No.

### LVI.D — Los dos KPI, medidos con números

**NEURO-S2** — *Iniciativas completadas* = completadas ÷ propuestas este mes, `conteo`, >70%. El denominador **lo pone el cliente y le premia ponerlo bajo**. Simulado sobre Javier Alonso (Alonso Instalaciones, climatización, 7 empleados, 38.000 €/mes, 13 frentes abiertos en 12 meses y 1 cerrado):
...
Es el fraude aritmético de §LV.J otra vez, por la otra punta: el honesto no llega y el que promete poco se lleva el alta. Además la moneda está mezclada (r2 suma *decisiones de terminar*, r4 suma *prioridades semanales*, al mismo contador de "iniciativas del mes") y **4 de 6 ramas no alimentan nada** — si el cliente se lleva de C2 el frente 1, 3, 5 o 6, su KPI no se mueve nunca (riesgo §XXXI, el que §LV.H cerró en S1).

**NEURO-S3** — *Margen neto anual* = beneficio ÷ facturación, `estructural`, >15% **absoluto**. La agencia del propio `example` del síntoma parte del 8% (24.000 € sobre 300.000 €). `readyForAlta` exige `alcanzoObjetivo` (`TreatmentPage.tsx:9099`), o sea margen ≥15% → **45.000 € de beneficio sobre la misma facturación, +87,5% en un ciclo**. Alta inalcanzable por diseño, el problema exacto de §LV.B, y NEURO-S3 no lleva `kpi_objetivo_puntos`. Con `kpi_objetivo_puntos: 4` (patrón UCI-S3, §LIV.C) el listón pasa a >12,0% sobre su punto de partida y se alcanza con 36.000 €. Los `threshold_*` siguen en 70/85/95/100, heredados de un KPI viejo — muertos en el frontend (declarados en `TreatmentPage.tsx:81-84` y no leídos por nadie), pero mienten en el dato.

### LVI.E — La contradicción de `r6` de NEURO-S3, resuelta: §XLVIII.B se equivocó

§XLVIII.B lo marcó como uno de los 4 críticos: *"de 9 campos, solo 3 alimentan alguna fórmula"*. **Leídas las fórmulas de verdad, los 9 alimentan alguna**: los 6 de diagnóstico entran todos en `indice_urgencia` y los 3 de cronograma en `margen_actual_12m` / `coste_total_espera_12m`. El evaluador (`evaluarFormula`, `TreatmentPage.tsx:506`) soporta paréntesis y menos unario, así que la expresión se evalúa entera. Verificado además que el JSON no ha cambiado desde entonces (`symptoms.bak_pre_neuros1_20260822.json` trae la misma fórmula, y ningún commit tocó NEURO-S3).

**Pero `r6` sí está roto, por otro sitio y menos visible:**
- `coste_total_espera_12m` se reduce algebraicamente a `ingresos × mejora_esperada_pct/100 × 12`. El "coste de esperar" es **exactamente lo que el cliente tecleó** en *"Mejora esperada si rediseña %"*, con dos campos más de decorado. Es el criterio 5 de §LV.L: el euro no se teclea, se deriva de un antes y un después.
- `indice_urgencia` devuelve un número sin escala y sin `semaforo`: *"más alto = más urgente"* sin decir más urgente que qué. Criterio 9 (veredicto).

### LVI.F — Solape NEURO-S1.r1 ↔ NEURO-S3.r3: sí se pisan

NEURO-S1.r1 mueve precio y saca *"De más al año (€)" = (después − hoy) × veces*. NEURO-S3.r3 *"Simulador de subida de precios"* mueve precio y saca *"Impacto neto (€)" = ingresos nuevos − ingresos actuales*. **Es la misma palanca y la misma cifra**, con la diferencia de que r3 estima además clientes perdidos. Y r3 calcula **ingresos**, no margen: no sirve al KPI de su propio síntoma (subir precio sube el margen, pero la tabla no lo enseña). Reportado, no resuelto — decisión de Maite.

### LVI.G — Duplicación por significado, confirmada en los dos (§LV.F sigue mandando)

Leídas por significado, no por vocabulario:
- **NEURO-S2**: *"trabajo con intensidad pero no avanzo en lo que importa"* ≡ *"todo parece prioritario así que nada lo es"* ≡ *"me cuesta decidir qué va primero"* — **tres formas de decir "no tengo criterio de prioridad"**. Y *"no tengo tiempo protegido para pensar el negocio"* es la causa 5 de NEURO-S1 (*"no reservo tiempo para dirigir"*) literalmente repetida en otro síntoma.
- **NEURO-S3**: *"tengo líneas cuya rentabilidad nunca he calculado"* ≡ *"no analizo el margen real de cada servicio"*. Y las ramas `r1`/`r4` piden las mismas tres cifras (facturación, margen, clientes) con envoltorio distinto — la redundancia de §XLVIII.B, ahora con el mecanismo visto.

El linter sigue sin poder verlo, igual que en §LV.F.

### LVI.H — Propuestas abiertas, a la espera del OK

1. **KPI de NEURO-S2 → Frentes cerrados** = frentes cerrados ÷ frentes abiertos en los últimos 12 meses, `conteo`, >70%. Los dos números los tiene y ninguno se inventa; el denominador es historia, no aspiración. Javier: 13 abiertos y 1 cerrado → 7,7%; cerrando 9 en el ciclo → 76,9% y alta. Cerrando 7 → 61,5% y no hay alta. Verificado con el motor real (`(InputA/InputB)*100`, sin código nuevo).
2. **Las 6 causas de NEURO-S2**, sin reducir el número: los tres duplicados se funden en uno, *"tiempo protegido"* vuelve a NEURO-S1 (donde ya vive, con su rama y su euro) y entran las tres que hoy tienen herramienta pero no causa — reuniones sin decisión, decisiones que suben al dueño, y **el equipo que llega de §LV.K**. Su herramienta (r1, el planificador de agenda) se mueve con su causa a NEURO-S1 en vez de borrarse.
3. **C2 de NEURO-S3**: reescribir los 6 dilemas al patrón de la casa para árbol.
4. **`kpi_objetivo_puntos: 4`** en NEURO-S3 y `threshold_*` rebasados.
5. **`r6` de NEURO-S3**: quitar el campo de mejora inventada y darle veredicto al índice.
6. **`r1`/`r4` de NEURO-S3**: r4 deja de repreguntar las cifras de r1 y las hereda.
...
*§LVI añadida en sesión 22 ago 2026 (cont.) — NEURO-S2 y NEURO-S3 con los 15 criterios de §LV.L. Aplicado el PASO 0, **los dos salen 🔴 y por el mismo sitio: C1→C2**, así que no se repite la pasada de ramas de NEURO-S1 — arreglar tarjetas sobre una cadena rota es el trabajo tirado que avisa la regla de oro. En NEURO-S2 el motor empareja la causa de C1 con `capa_2_options[idx]` (`TreatmentPage.tsx:3503`) y los dos arrays cuentan historias distintas en el mismo orden: quien marca *"tengo demasiados proyectos a medias"* recibe *"bloquear tiempo para la estrategia"* y un planificador de agenda — el mecanismo crítico de RES-S1, aquí en el índice 0. En NEURO-S3 las 6 `capa_2_options` son disyuntivas *"¿A o B?"* dentro de una familia árbol que solo sabe contestar Sí/No, y ese texto titula el frente de C3 — el patrón crítico de OPE-S3, y NEURO-S3 es el único de los 6 síntomas árbol que lo hace. Los dos KPI medidos con números: el de S2 premia al que promete poco (dice 7 y cierra 3 → 57%, sin alta; dice 2 y cierra 1 → 100% y alta), mezcla monedas y deja 4 de 6 ramas sin alimentar nada; el de S3 exige un margen absoluto del 15% a quien parte del 8%, o sea +87,5% de beneficio en un ciclo, sin `kpi_objetivo_puntos` que lo rebaje. **Resuelta la contradicción de §LV.M sobre NEURO-S3.r6: §XLVIII.B se equivocó** — los 9 campos alimentan fórmula (6 el `indice_urgencia`, 3 el cronograma) y el evaluador soporta la expresión entera; el defecto real es otro, `coste_total_espera_12m` se reduce a `ingresos × mejora_esperada/100 × 12`, o sea el "coste de esperar" es literalmente el número que el cliente se inventó, y el índice sale sin escala ni veredicto. **Solape S1.r1 ↔ S3.r3 confirmado**: misma palanca (precio) y misma cifra, y r3 además calcula ingresos y no margen, así que no sirve al KPI de su propio síntoma — reportado, no resuelto. Duplicación por significado confirmada en los dos (§LV.F): tres causas de S2 dicen "no tengo criterio de prioridad" y una cuarta es literalmente la causa 5 de NEURO-S1; en S3 dos causas dicen "no calculo el margen por servicio" y r1/r4 piden las mismas cifras. Aplicado solo lo mecánico ya aprobado — 52 cambios: `filas_iniciales` 20→1, `vista:tarjeta` en 5 secciones, 20 primeras opciones vacías, la colisión de regex de r5 y `no_sumar` en 22 columnas (r3 de S3 sumaba 8 a la vez) — masframe@3d5f209. Todo lo que es criterio (KPI de S2, las 6 causas con la del equipo que llega de §LV.K, C2 de S3, `kpi_objetivo_puntos`, r6, r1/r4) queda propuesto en §LVI.H y a la espera del OK*
