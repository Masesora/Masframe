...
### LVI.H — El patrón que fija `masframe@5b22f31`, y que estos dos también incumplen

Mientras se cerraba este análisis, NEURO-S1 recibió en paralelo la corrección que hace explícito el patrón: **I-1 estaba roto** — sus 6 `capa_2_options` eran las 6 prescriptivas, ninguna de diagnóstico — y además eran *"seis sistemas de gestión que instalar, no acciones que corrijan los inputs de C0"*. Como son pasos de una misma secuencia y no compiten entre sí, la matriz de impacto/esfuerzo llegaba a proponer *"revisar cada trimestre"* — el último paso — como primera decisión. Quedó así:

- **1-3 son lo que la herramienta le ENSEÑA**: *"Ver cómo ha evolucionado tu beneficio…"*, *"Ver qué te deja cada línea…"*, *"Ver a dónde se te va el dinero…"*
- **4-6 son movimientos que se EJECUTAN y bajan a C4**: *"Escribir los 3 objetivos…"*, *"Soltar horas de operativa…"*, *"Dejar de hacer lo que no te acerca…"*
- **`capa_2_decision`: "Urgente vs Importante" → "Regla de prioridades"** — C2 deja de puntuar y pasa a descartar; lo que sobrevive baja a C3 en su orden natural, casando 1:1 con su causa de C1.

**Los dos síntomas de esta sección incumplen lo mismo.** NEURO-S2 es el caso idéntico: `capa_2_decision: "Urgente vs Importante"`, seis opciones todas prescriptivas y todas pasos de una misma secuencia que no compiten. NEURO-S3 tiene las seis en forma de disyuntiva, que no es ni diagnóstico ni prescripción.

### LVI.I — El rediseño propuesto, a la espera del OK

**NEURO-S2 — C0 auditable + I-1.** `InputA = frentes que has cerrado en los últimos 12 meses` · `InputB = frentes que has tenido abiertos en los últimos 12 meses`. Los dos son hechos y **el inventario de r1 es el documento que los respalda**, que es lo que la garantía necesita. KPI *Frentes cerrados*, `conteo`, `(InputA/InputB)*100`, >70 %, umbrales 0/35/70/85. Javier Alonso: 13 y 1 → 7,7 %; cerrando 9 → 76,9 % y alta; cerrando 7 → 61,5 % y no hay alta. Verificado con el motor real, sin código nuevo. `capa_2_decision` pasa a **"Regla de prioridades"**.

| # | C1 (la causa) | C2 | C3 |
|---|---|---|---|
| 1 | Tengo cosas empezadas hace meses y ninguna termina | **Ver** cuántos frentes tienes abiertos, desde cuándo y qué falta para cerrar cada uno | Inventario de frentes abiertos |
| 2 | Cada semana entra algo urgente que descoloca lo planificado | **Ver** cuántas horas de tu semana se van en urgencias que no te necesitaban | Las urgencias que te roban la semana |
| 3 | Me paso el día en reuniones de las que no sale ninguna decisión | **Ver** qué reuniones se comen tu mes sin producir una sola decisión | Auditoría de reuniones |
| 4 | Todo me parece prioritario y al final no cierro nada | **Cerrar o matar** cada frente abierto, con fecha y por escrito | Cerrar o matar, uno a uno |
...
| 6 | Cada uno interpreta a su manera lo que hay que hacer | **Poner por escrito** las 3 prioridades del negocio y que cada uno diga cuál es la suya | Dirección común del equipo |

La 6 es la causa que llega de §LV.K. Sale *"tiempo protegido para dirigir"*, que ya vive en NEURO-S1 con su rama y su euro — y **su herramienta (el planificador de agenda semanal) se muda con ella a NEURO-S1**, no se borra.

**NEURO-S3 — I-1, y el input compartido se precarga en vez de cambiarse.** Sobre el solape con NEURO-S1: la tercera opción no es inventarle a S3 otro input, es **precargar** el beneficio del año desde el C0 de S1 cuando el cliente lleva los dos síntomas — el estándar §XV.B que el propio linter ya recomienda para columnas que repreguntan un input de C0. Se mantiene la medida, se quita la fricción. `kpi_objetivo_puntos: 4` (patrón UCI-S3, §LIV.C) → listón a >12,0 % sobre su punto de partida, alcanzable con 36.000 € frente a los 45.000 € de hoy. Y **fuera el modo `estructural`** (precedente §LIV): si cada rama entrega € de beneficio de más al año, C6 se mueve con lo comprometido en C3 y dado por hecho en C4, sin pedirle que reescriba las cuentas del ejercicio.

| # | C1 (la causa) | C2 | qué cifra entrega la rama |
|---|---|---|---|
| 1 | Facturo más cada año pero no gano más en proporción | **Ver** qué margen te deja de verdad cada servicio, uno a uno | — |
| 2 | Trabajo más horas para mantener los mismos ingresos | **Ver** cuánto margen deja cada hora que echa el equipo | — |
| 3 | Sigo ofreciendo cosas por inercia, sin saber si son rentables | **Ver** qué clientes y qué líneas consumen más de lo que dejan | — |
| 4 | Tengo servicios que no cubren ni su coste | **Retirar o repreciar** los que no cubren coste | (precio nuevo − precio hoy) × veces, o la pérdida que dejas de asumir |
| 5 | Mi catálogo es tan amplio que dispersa recursos | **Concentrar** la venta en las líneas de más margen | margen de más por cada venta desplazada |
| 6 | Cada vez que crezco me trae más gasto y no más beneficio | **Bajar el coste** de lo que sí es rentable: automatizar o renegociar | (coste hoy − coste nuevo) × veces al año |

La causa 6 es **nueva** y sustituye al duplicado *"no analizo el margen real de cada servicio"* (≡ causa 3): es el mecanismo que da nombre al síntoma y hoy no está en C1. No se reduce el número de causas.
...
## LVII. SESIÓN 22 AGO 2026 (cont.) — Las instrucciones precisas, y el cambio de unidad de trabajo

La jornada de NEURO-S1 acabó con el diagnóstico del propio usuario: **"este ritmo es insufrible"**. Y tenía razón — el síntoma quedó bien, pero costó tres reanclajes de KPI, dos rediseños de causas y cinco correcciones sobre lo mismo. El problema no fue la calidad del trabajo: fue **el goteo**. Una tarjeta, una pregunta, una corrección, otra vez.

### LVII.A — Las reglas que salieron de la sesión, escritas como reglas

Recogidas en la skill (§CRITERIOS) y en memoria, para que no dependan de que alguien se acuerde:

**Método**
1. **La cadena clínica primero** (§LV.A). Objetivo → KPI → C1..C6 y la frase de cierre, antes de mirar un bug.
2. **Las reglas del PLAN son puerta previa.** Comprobar los invariantes *antes* de proponer y, si algo los rompe, **decirlo en el primer párrafo**. Nunca presentar como válida una propuesta que los incumple.
3. **`symptoms.json` es el libro maestro.** Los campos son sagrados: no se reinterpretan ni se les inventa significado.
4. **No recortar producto para que cuadre.** Si la propuesta quita algo o cambia lo que se mide, buscar antes la tercera opción.
...
### LVII.B — `data/auditar_sintoma.py`: el diagnóstico mecánico deja de hacerse a mano

Todo lo que en esta jornada se comprobó tecleando consultas una a una pasa a un script que acepta **un síntoma, una especialidad o `--todos`**: colisiones de `ACCION_REGEX`, qué suma el TOTAL, `contribuye_valor` de más, opciones pre-marcadas, fórmulas con claves inexistentes o divisiones por cero, secciones sin veredicto, `filas_iniciales` desproporcionado, tarjetas de 9+ campos, `confirmaciones` descuadradas, columnas que repreguntan un dato de C0, jerga y copy coloquial, y modo de recuperación incoherente.

**Calibrado contra NEURO antes de darlo por bueno**, y en esa calibración se quitaron dos clases de falso positivo que lo habrían hecho inservible: las columnas de acción (que `filaVaciaHerramienta` **ya** deja en blanco, así que avisar de ellas era gritar en falso en 12 secciones) y los KPI de conteo alimentados por `contribuye_valor_si` / `cuenta_unicos_si` / `suma_si`, no solo por `contribuye_valor`. El solape de vocabulario solo avisa cuando falla **la cadena entera** para el mismo índice — el vocabulario no ve el significado (§LV.F), así que por separado solo produce ruido.

Resultado sobre NEURO: 0 bloqueantes en los tres, y **encontró solo el hallazgo real de NEURO-S1.r2** que había costado media hora de lectura manual (la opción promete *"ver qué te deja cada línea"* y la herramienta pide un plan de unidades).

**Regla nueva:** cuando aparezca una comprobación que el script no hace, **se le añade al script**. Cierra la clase, no el caso.

### LVII.C — La unidad de trabajo pasa a ser la ESPECIALIDAD (skill v5)

De un síntoma por sesión a **tres**, en cuatro fases:

| Fase | Qué | Cómo |
|---|---|---|
| **1** | Cadena clínica de los 3 | Juicio. No se automatiza. Media página cada uno |
| **2** | Diagnóstico mecánico | `python data/auditar_sintoma.py <ESPECIALIDAD>` + el linter de contratos |
| **3** | **UNA sola ronda de decisiones** | Todas las preguntas de criterio de los 3, en un único bloque, cada una con recomendación, alternativa y ejemplo con nombre y números |
| **4** | Aplicar y verificar | Un commit por síntoma, con las puertas de salida |

**Prohibido volver a preguntar hasta haber aplicado lo confirmado.** Si aparece una duda nueva a mitad, se anota y se sigue con todo lo demás.

**Puertas de salida obligatorias:** `auditar_sintoma.py` sin bloqueantes · `validar_sintomas.py` sin errores nuevos · `tsc --noEmit` limpio · round-trip idéntico del JSON · una simulación numérica de C0→C6 que demuestre que el alta se alcanza con trabajo real · sección en el plan.

La skill instalada pasa a **v5** y lleva dentro los 17 criterios de §LVII.A, para no tener que ser corregida otra vez sobre lo mismo. Copia versionada: `masframe-ux-validator_SKILL_v5_INSTALADA.md`.

### LVII.D — Pendiente

- **El enlace de conversación que el usuario quería incorporar** (`claude.ai/share/c746578e…`) **no se pudo leer**: devuelve la cáscara de la aplicación, sin contenido. Queda pendiente de que se pegue el texto para incorporarlo a la skill.
- Las especialidades sin pasar por el método nuevo: **UNI, CARDIO, CLI, CIR, PSI, RES, TER, OPE**. NEURO queda a medio camino — S1 cerrado, S2 y S3 con el trabajo mecánico aplicado (§LVI) pero sin la pasada de criterio.
- Sigue abierto llevar la causa del equipo de NEURO-S1 a **NEURO-S2**.
