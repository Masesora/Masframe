...
def build_triaje_for_code(client_data):
    ...
    # Lista única de especialidades técnicas desde symptoms.json
    specialties = sorted(set(s["specialty"] for s in all_symptoms))

    for technical in specialties:

        clinical_name = technical  # nombre visible
        ...
        if any(s["thresholds"].get("critical") for s in sintomas_output):
            ...
            # 2.4 Construir bloque de especialidad
        especialidades_output.append({
            "nombre": clinical_name,
            "technical": technical,
            "icon": "",
            "department": SPECIALTY_TO_DEPARTMENT.get(clinical_name, "GENERAL"),
            "short_description": "",
            "explanation": "",
            "color": color,
